...
=======
OmniAI Search 是一款聚合式浏览器 AI 助手扩展。当你在传统搜索引擎结果页浏览内容，或在任意网页中划词、右键触发 AI 处理时，它会在网页悬浮小窗或原生侧边栏中联动多个主流 AI 平台，并将问题定向发送到对应对话，让你在阅读、搜索、对比和总结时无缝接入 AI 能力。
>>>>>>> 86973dd (feat: add Z.AI and context menu platform settings)
...
- 🧠 Z.AI
- 🤝 元宝
- 🧩 智谱清言 / Gemini / Claude / Perplexity / Copilot / Grok
>>>>>>> 86973dd (feat: add Z.AI and context menu platform settings)
...
3. **调整平台与右键默认值**：点击小窗右上角的“⚙️ 设置”图标，可开启或关闭需要展示的平台，并在设置页单独指定右键“使用 AI 处理”的默认平台。
...
- **核心机制**：通过本地队列、`chrome.storage` 与 `postMessage` 实现小窗、原生侧边栏与 iframe 之间的定向通信；采用 `document.documentElement` 挂载方案，尽量降低搜索引擎 SPA 局部刷新对界面的影响。
>>>>>>> 86973dd (feat: add Z.AI and context menu platform settings)
