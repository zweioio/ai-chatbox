...
ai-chatbox 是一款面向 Chromium 浏览器的聚合式 AI 助手扩展。它把传统搜索引擎、网页划词、右键菜单、悬浮小窗、原生侧边栏、内容对比、提示词中心和本地备忘录整合到同一套工作流中，让用户不必在多个 AI 网站之间来回切换，就能把当前问题快速发往指定平台并继续对话。

这个项目的核心目标不是简单地“把几个 AI 网站塞进 iframe”，而是围绕真实使用场景重新组织交互链路：当你正在搜索结果页查资料时，可以直接在页面侧边调出 AI；当你在任意网页阅读文章、文档或代码时，可以通过划词或右键把选中内容立即发送到 AI；当你需要比较不同模型回答差异时，可以一键打开内容对比页；当你想沉淀重点回答时，可以直接保存到备忘录并导出图片；当你有高频固定任务时，可以通过提示词中心和快捷动作把这些操作沉淀成稳定工作流。
...
ai-chatbox 的优势就在于：它把这些高频、琐碎、重复的动作统一到浏览器内部完成。你不用离开当前页面，也不用重新整理上下文，就可以把“搜索 → 阅读 → 提问 → 对比 → 收藏”变成一条连续流程。
...
同时支持差异洞察、本地保存、按窗口收藏到备忘录等能力，适合做更严肃的信息整理和二次沉淀。
...
- 当前回答、对比内容都可以保存到本地备忘录
- 备忘录支持独立查看、导出对话图片、按来源平台整理

这意味着它不仅是一个“临时提问入口”，更是一个可以长期积累工作流、素材和常用提问方式的工具。
...
- 可并排比较 1～4 个 AI 平台结果
- 支持平台切换、窗口刷新、按窗口保存到备忘录、摘要对照
- 更适合深度阅读、整理和筛选答案

#### 本地收藏 / 备忘录

- 可保存当前回答和对比内容
- 支持书架式列表查看、阅读宽度切换、导出对话图片
- 便于沉淀重要结论、优秀提示词和对比结果
...
如果把 ai-chatbox 和一般“搜索页加一个 AI 面板”的扩展相比，它的优势主要在于：
...
ai-chatbox is a browser extension for Chromium-based browsers that turns search pages, text selection, context menus, floating panels, native side panels, prompt workflows, memo capture, and multi-model comparison into one unified AI workspace.

Instead of forcing users to copy text, open multiple AI sites, and repeat the same question over and over again, the extension keeps the workflow inside the browser. You can search, read, highlight text, send it to a selected AI platform, compare answers, save useful results to a memo library, and export conversation images without constantly breaking your focus.
...
ai-chatbox is designed to solve exactly those workflow problems. It is not only an “AI panel on top of search.” It is a practical browser-side productivity layer for reading, asking, comparing, and organizing AI-assisted work.
...
ai-chatbox is intentionally lightweight and practical:
...
Compared with many “AI beside search” tools, ai-chatbox has stronger real-world workflow coverage:
