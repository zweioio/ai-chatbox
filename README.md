...
OmniAI Search（当前扩展名为 AIChatbox）是一款面向 Chromium 浏览器的聚合式 AI 助手扩展。它把传统搜索引擎、网页划词、右键菜单、悬浮小窗、原生侧边栏、内容对比、提示词中心和本地备忘录整合到同一套工作流中，让用户不必在多个 AI 网站之间来回切换，就能把当前问题快速发往指定平台并继续对话。
...
OmniAI Search (currently packaged as AIChatbox) is a browser extension for Chromium-based browsers that turns search pages, text selection, context menus, floating panels, native side panels, prompt workflows, memo capture, and multi-model comparison into one unified AI workspace.
